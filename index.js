...
async function xybSign(config) {
    ...
    const login = async () => {
        ...
        if (config?.openId && config?.unionId) {
            const data = await $http.post(apis.loginWX, {
                openId: config.openId,
                unionId: config.unionId,
            });

            sessionId = data.sessionId;
            loginerId = data.loginerId;
        } else {
            // let data = await $http.post(apis.login, {
            ...
            // sessionId = data.sessionId;
            // loginerId = data.loginerId;
            throw new Error("请配置 openId 和 unionId");
        }
    ...
    const submitBlog = async (traineeId, blogTask) => {
        ...
        const id = await $http.post(apis.weelBlogSave, {
            ...
            blogBody: blogs[blogTask.week - 1],
            blogOpenType: 2,
            traineeId: traineeId,
            isDraft: 0,
